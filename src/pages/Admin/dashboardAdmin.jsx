            ...
            <table className='w-full table-auto'>
              <thead className='border-b-2 border-bozz-one'>
                {tableHead.map((title,index) => {
                  return <th className='text-bozz-one font-semibold' key={index}>{title}</th>
                })}
              </thead>
              <tbody>
                {datas.map((data, index) => {
                  return (
                    <tr className='text-bozz-one border-b-2 border-bozz-one h-10 text-center'>
                      <td>{index + 1}</td>
                      <td>{data.company_name}</td>
                      <td>{data.register_date}</td>
                      <td>{data.company_location}</td>
                      <td>{data.register_status}</td>
                      <td>{data.register_status === 'need verify' ? <button className='w-24 h-8 bg-bozz-one text-bozz-six rounded-lg'>Verify</button> : '-'}</td>
                    </tr>
                  )
                })}
              </tbody>
